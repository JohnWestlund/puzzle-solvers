...
def find_paths(grid, start, end, visited):
    if start == end:
    ...
    for dx, dy, direction in [(-1, 0, '^'), (1, 0, 'v'), (0, -1, '<'), (0, 1, '>')]:
        ...
        if is_valid_move(grid, nx, ny, visited):
            for path in find_paths(grid, (nx, ny), end, visited):
...
def find_all_paths(grid, pairs, index, visited, total_traversable):
    ...
    for path in find_paths(grid, start, end, visited):
...
def main():
    ...
    for i, (start, end) in enumerate(pairs):
        ...
        paths = find_paths(grid, start, end, set())
        pair_paths_count.append((len(paths), (start, end)))
        print(f"Pair {labels[i]} ({start} -> {end}): {len(paths)} possible paths")
